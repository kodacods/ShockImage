...
public class RectangularShowSelection implements ImageOperation, java.io.Serializable {
    ...
    public BufferedImage apply(BufferedImage input) {
        ...
        // g2d.drawRect(SelectionActions.originPoint.x, SelectionActions.originPoint.y,
        // SelectionActions.selWidth, SelectionActions.selHeight);
        // g2d.dispose();
        // Now doing a modified version of brightness modification.
        // Code partially copied from initial Brightness filter with that filter's
        // author's permission.
        int borderCornerX = 0;
        int borderCornerY = 0;

        // determine the point of the lower right corner as opposed to the already set
        // origin point
        ...
        for (int y = 0; y < input.getHeight(); y++) {
            for (int x = 0; x < input.getWidth(); x++) {
                ...
                if (x < borderCornerX && x > SelectionActions.originPoint.x && y < borderCornerY
                        && y > SelectionActions.originPoint.y) {
    ...
    /**
     * Takes the pixel value that has been adjusted to the new percentage
     * brightness.
     * If the pixel value is <0 the pixel value will be set to 0
     * If the pixel value is >255 the pixel value will be set to 255
     * If the pixel value is within the range of 0-255, then the pixel value will
     * not change.
     * 
     * @param value
     * @return value
